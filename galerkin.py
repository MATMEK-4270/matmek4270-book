...
class FunctionSpace:
    ...
    def eval(self, uh, xj):
        ...
        P = self.evaluate_basis_all(Xj)
        return P @ uh

    def evaluate_basis_all(self, Xj):
...
class Dirichlet:
    ...
    def __init__(self, bc, domain, reference_domain):
        ...
        h = d[1]-d[0]
        self.bc = bc
        self.x = bc[0]*(d[1]-x)/h + bc[1]*(x-d[0]) / \
            h           # in physical coordinates
        self.xX = map_expression_true_domain(
            self.x, x, d, r)  # in reference coordinates
...
class Neumann:
    ...
    def __init__(self, bc, domain, reference_domain):
        ...
        self.x = bc[0]/h*(d[1]*x-x**2/2) + bc[1]/h * \
            (x**2/2-d[0]*x)  # in physical coordinates
        self.xX = map_expression_true_domain(
            self.x, x, d, r)      # in reference coordinates
        self.Xl = sp.lambdify(x, self.xX)
...
class NeumannLegendre(Composite, Legendre):
    def __init__(self, N, domain=(-1, 1), bc=(0, 0), constraint=0):
        ...
        self.B = Neumann(bc, domain, self.reference_domain)
        k = np.arange(N+1)
        self.S = sparse.diags((1, -k*(k+1)/(k+2)/(k+3)),
                              (0, 2), shape=(N+1, N+3), format='csr')
...
class NeumannChebyshev(Composite, Chebyshev):
    def __init__(self, N, domain=(-1, 1), bc=(0, 0), constraint=0):
        ...
        self.B = Neumann(bc, domain, self.reference_domain)
        k = np.arange(N+1)
        self.S = sparse.diags((1, -(k/(k+2))**2), (0, 2),
                              shape=(N+1, N+3), format='csr')
